const { deriveListingState, applyFilters } = require('../../cunzhen-miniapp/utils/listing')
const { readState, writeState, resetState } = require('./persistence')
...
function getAllListings() {
  return readState().listings.map(deriveListingState)
}

function getListingById(id) {
  const listing = readState().listings.find(function (item) {
    return item.id === id
  })
...
function createViewingAppointment(payload) {
  const state = readState()
  const listing = state.listings.find(function (item) {
    return item.id === payload.listingId
  })
  const viewing = {
    id: createViewingId(state.viewings),
    listingId: payload.listingId,
    listingTitle: payload.listingTitle || (listing ? listing.title : ''),
  ...
  state.listings = state.listings.map(function (item) {
    if (item.id === payload.listingId && item.status === '可发布') {
      return Object.assign({}, item, { status: '已预约' })
    }
    return item
  })
  writeState(state)
...
function getAdminSummary() {
  ...
  return {
    publishableCount: listings.filter(function (item) { return item.frontendVisible }).length,
    ...
    leadStatusCount: countBy(state.leads, 'status'),
    viewingStatusCount: countBy(state.viewings, 'status'),
    latestListings: listings.slice(0, 6),
    latestLeads: state.leads.slice(0, 6),
    latestViewings: state.viewings.slice(0, 6),
    riskyListings: listings.filter(function (item) { return item.expired || !item.publishable }).slice(0, 6)
  }
}
